...
void decisiontree(cv::Mat& trainingData, cv::Mat& trainingClasses, cv::Mat& testData, cv::Mat& testClasses) {
    ...
    //type must be 8uc1,8sc1,32sc1. if type is one of 8uc1,8sc1, the varIdx,samIdx should be the mask.if type is 32sc1, they are the index.
    int dt[2] = {0,1};
    Mat varIdx(1,2,CV_32SC1,dt);
    cout <<"varIdx:"<<varIdx<<endl;

    Mat samIdx(1,trainingData.rows,CV_8UC1,Scalar(1));
    cout <<"samIdx:"<<samIdx<<endl;

    //mask should as large as traindata, and the type must be one of 8uc1,8sc1,32sf1.
    Mat mismask(trainingData.rows,trainingData.cols,CV_8UC1,Scalar(0));
    cout <<"msimask:"<<mismask<<endl;
    ...
    dtree.train(trainingData,CV_ROW_SAMPLE, trainingClasses, varIdx, samIdx, var_type, mismask, CvDTreeParams());
    //dtree.save("tree.xml","test_tree");
